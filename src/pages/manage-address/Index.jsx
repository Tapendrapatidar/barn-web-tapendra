...
import {
  Box, Text, Flex, FormControl, FormLabel, Input, Heading, Image, HStack, Button, RadioGroup, Radio, VStack, Spinner
} from '@chakra-ui/react';
import { ProfileSideBarComponent } from '../../components/profileSideBarComponent/Index'
import { useDispatch, useSelector } from 'react-redux';
import toast from 'react-hot-toast';
import { ArrowForwardIcon, DeleteIcon, EditIcon, SmallAddIcon, SmallCloseIcon } from '@chakra-ui/icons';
  ...
  return (
    <Box minH={'80vh'} pos={'relative'}>
      ...
      >
        ...
        <Flex mt={4} w={{ base: "100%", lg: "100%" }} h={{ base: "fit-content", xl: "640px" }} bg={'white'} border={"1px solid #cccccc"} rounded={'md'} boxShadow={'md'} flexDir={'column'} gap={2}>
          <Flex p={{ base: "4", md: "4" }} gap={2} flexDirection={'column'}>
            <HStack w={'100%'} justify={'space-between'}>

              <Heading w={'80%'} fontSize={'20px'} fontWeight={'500'} className='pc1' primary={"primary"}>Manage Address</Heading>
              <Box display={{ lg: "none" }}>
                <ProfileSideBarComponent />
              </Box>
            </HStack>
            ...
            >
              ...
              <>
                {
                  !id ?
                    <>
                      <Box cursor={'pointer'} display={data.length <= 0 ? "none" : "block"} onClick={() => setAddressActive(!addressActive)} fontWeight={'600'} textColor={"#2b8f65"}>  <SmallAddIcon fontSize={20} />  Add New Address
                      </Box>
                    ...
                    :
                    <Box fontWeight={'600'} textColor={"#2b8f65"}>Update Address</Box>
                }
                <Box display={!data.length <= 0 ? "none" : "block"} fontWeight={'600'} textColor={"#2b8f65"}>Add New Address</Box>

              </>


              <Box display={addressLoading || !data.length <= 0 && !addressActive ? "none" : "inline-block"}>
              ...
              </Box>
              {
                addressLoading ?
                  ...
                  : <Box display={addressActive ? 'none' : 'inline-block'} flexDirection={'column'} gap={3}>
                    {
...

          {
            checkOut && (
              <Button onClick={() => {
                if (rental) {
                  navigate(`/check-out?&rental=${rental}&quantity=${quantity}&productId=${productId}`);
                } else if (service) {
                  navigate(`/check-out?&service=${service}&servicePurchaseDay=${servicePurchaseDay}&serviceId=${serviceId}`);
                } else {
                  navigate(`/check-out?`);

                }
              }
              } variant={'outline'} textColor={'#2B8F65'} colorScheme='blue' pos={'absolute'} bottom={20} right={20}> Checkout Now {''} <ArrowForwardIcon ml={2} /> </Button>
            )
          }
        </Flex>
...
const ViewAddressComponent = ({ item, addressActive, setAddressActive, selectedAddressId, handleRadioChange, getAllAddress }) => {
  const navigate = useNavigate()
    ...
    <Box p={2} borderBottom={'1px solid #cccccc'} w={'100%'} >
      <HStack w={'100%'} justifyContent={'space-between'}>
        <Box>
          ...
          <Text fontSize={'14px'} noOfLines={2} textColor={'#222222'} fontFamily={'Sofia Sans'}>
            {
        ...
        <VStack>
          <Button onClick={() => {
            ...
            colorScheme='teal' variant={'outline'} gap={1} w={20} justify={'space-between'} cursor={'pointer'} fontWeight={'bold'} textColor={'#00a3e2'}>
            Edit
            <EditIcon />
          </Button>
          <Button gap={1} colorScheme='teal' variant={'outline'} isDisabled={loadingDelete} onClick={() => deleteAddressHandler(item.addressId)} w={20} justify={'space-between'} cursor={'pointer'} fontWeight={'bold'} textColor={'#ff7f7f'}>
            Delete
            <DeleteIcon />
          </Button>
        </VStack>
