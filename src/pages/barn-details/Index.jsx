...
import { ArrowBackIcon, ArrowForwardIcon, StarIcon, ViewIcon } from '@chakra-ui/icons'
  ...
  const renderCustomPrevArrow = (onClickHandler, hasPrev) => {
    return (
      <Box
        ...
        disabled={!hasPrev}
      >
        <ArrowBackIcon fontSize={'20px'} mt={{ base: "0", md: "10px" }} fontWeight={'bold'} textColor={'black'} />
      </Box>
    );
  };
  ...
  const renderCustomNextArrow = (onClickHandler, hasNext) => {
    return (
      <Box
        ...
        disabled={!hasNext}
      >
        <ArrowForwardIcon fontSize={'20px'} mt={{ base: "0", md: "10px" }} fontWeight={'bold'} textColor={'black'} />
      </Box>
    );
  };
        ...
        <HStack w={{ base: "98%", md: "98%" }}
          ...
          <Box w={{ base: "98%", md: "98%" }} >
            <HStack pb={2} justify={'space-between'} flexWrap={'wrap'} mx={2}>
              ...
              <Button onClick={() =>
                navigate(`/check-out?isBarn=${true}&barnId=${barnData?.barnId}`)
              } w={{ base: "100%", md: '40' }} bg={'#2B8F65'} textColor={'white'} colorScheme='' variant={'outline'}>Need Stall</Button>
            </HStack>
            <Box border={'1px solid #cccccc'} rounded={'xl'} overflow={'hidden'} w={'100%'}  >
            ...
            >
              ...
              >
                ...
                <Box p={3}>
                  ...
                  >
                    ...
                    {barnData?.boarding?.stallFeatures?.matted && (
                      <BarnDetails src={mattedIcon} name={"matted"} name2={"yes"} />
                    )}
                ...
                <Text textColor={"#666666"}>{barnData?.description}</Text>
                <Box border={'1px solid #cccccc'} rounded={'xl'} overflow={'hidden'}>
                ...
                >
                  {
                    barnData?.nearBy?.map((item, index) => (
                      <Text key={index}>{item?.place} - {item?.distance}</Text>
                    ))
                  }
                ...
                {reviewList?.map((item, index) => {
                  return (
                    <Box p={2}>
                      ...
                      <Text noOfLines={2} ml={2} fontWeight={500} fontSize={"18px"}>
                        {item?.description}
                      </Text>
                    </Box>
                  );
  ...
  return (
    <HStack w={'100%'} justify={'space-between'}>
      <HStack>
        ...
        <Text fontSize={{ base: "13px", md: "18px" }} fontWeight={'500'} textColor={'black'}>{name}</Text>
      </HStack>
...
const ReviewModal = ({ isOpen, onClose, onOpen, submitRatingHandler, handleRatingClick, setDescription, description, setRating, rating, loadingRating }) => {
  ...
  return (
    <>
      ...
      <Text onClick={() => {
        ...
        onOpen();
      }} as={'h2'} fontSize={{ base: "14", md: "18" }} textColor={'#2B8F65'} fontWeight={'bold'}>
        + Add Reveiw
      </Text>
          ...
          <ModalBody rounded={'2xl'}>
            <Box>
              <div className="">
                <div className="">
                  <div>
                    <Text textColor={'#222222'} fontWeight={'semibold'} textAlign={'center'}> What is you rate? </Text>
                    ...
                    <Text textAlign={'center'}> Please share your opinion <br />
            ...
            <HStack justifyContent={'end'}>
              <Box justifySelf={'end'} justifyContent={'end'}>
                <Button mr={3} h={8} colorScheme='teal' variant={'outline'} border={'1px solid #FF3D00'} textColor={'#FF3D00'} onClick={onClose}>Cancel</Button>
                <Button
                  colorScheme=''
                  aria-required
                  h={'8'}
  ...
  return (
    <>
      ...
      <Button onClick={() => {
        setOverlay(<OverlayOne />);
        onOpen();
      }} variant={'outline'} border={'1px solid #cccccc'} fontSize={{ base: "12", md: "16" }} textColor={'black'} fontWeight={'bold'}>
        Show All  Amenities
      </Button>
        ...
        <ModalContent p={2} bg={'#FAFAFA'} rounded={'2xl'}>
          ...
          <ModalBody rounded={'2xl'}>
            <Box h={'600px'} className='hide-scrollbar' overflowY={'scroll'}>
              <Text as={'h2'} fontSize={{ base: "20", md: "28" }} textColor={'#000000'} fontWeight={'bold'}>
                Property utility
              </Text>
              <Box p={3}>
                <Grid w={{ base: "100%", md: "100%" }} templateColumns={{ base: "repeat(1,1fr)", md: "repeat(2,1fr)" }} gap={8}>

                  {facilities?.washStalls?.indoor &&
                    <BarnDetails src={IndoreIcon} name={'Indoor Arena Size'} name2={facilities?.indoorArena?.size} />}
                  {facilities?.washStalls?.outdoor &&
                    <BarnDetails src={OutdoorIcon} name={'Outdoor Arena Size'} name2={facilities?.outdoorArena?.size} />}
                  {/* {facilities?.washStalls?.lightedOutdoorArena && */
                    <BarnDetails src={LightedOutdorr} name={'Lighted Outdoor Size'} name2={facilities?.lightedOutdoorArena?.size} />}
                  {facilities?.roundPen &&
                    <BarnDetails src={RoundPenIcon} name={'Round Pen'} name2={'yes'} />}
                  {facilities?.tackRoom &&
                    <BarnDetails src={TackRoom} name={'Tack Room'}
                      name2={
                        facilities?.tackRoom?.shared && facilities?.tackRoom?.individualLockers
                  ...
                  {facilities?.trailerParking &&
                    <BarnDetails src={TrainlerParking} name={'Trailer Parking'}
                      name2={
                        facilities?.trailerParking?.included && facilities?.trailerParking?.fee
                          ? 'included / fee'
                  ...
                  {facilities?.washStalls?.heated &&
                    <BarnDetails src={heatedWash} name={'Heated Wash Stall'} name2={'yes'} />}
                  {facilities?.viewingLounge &&
                    <BarnDetails src={LoungeIcon} name={'Viewing Lounge'} name2={'yes'} />}
                  {facilities?.washStalls?.hotWater &&
                    <BarnDetails src={HotWater} name={'Hot Water'} name2={'yes'} />}
                  {facilities?.washStalls?.coldWater &&
                    <BarnDetails src={ColdWater} name={'Cold Water'} name2={'yes'} />}
              ...
              <Box p={3}>
                <Grid w={{ base: "100%", md: "100%" }} templateColumns={{ base: "repeat(1,1fr)", md: "repeat(2,1fr)" }} gap={8}>
                  {services?.laundryFacilities &&
                    <BarnDetails src={LaundryIcon} name={'Laundry Facilities'} name2={"yes"} />}
                  {services?.vendingMachines &&
                    <BarnDetails src={VendingMachineIcon} name={'Vending Machines'} name2={"yes"} />}
                  {services?.guestWiFi &&
                    <BarnDetails src={Wifi} name={'Guest Wifi'} name2={"yes"} />}
                  {!services?.dailyTurnout &&
                    <BarnDetails src={dailyTurnout} name={'Daily Turnout'} name2={"yes"} />}
