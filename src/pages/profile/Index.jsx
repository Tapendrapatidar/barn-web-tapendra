...
import {
    Box, Text, Flex, FormControl, FormLabel, Input, Heading, Image, HStack, Button, InputRightElement, InputGroup
...
function Index() {
    ...
    const handleChange = (e) => {
        ...
        switch (name) {
            ...
            case 'email':
                ...
                break;
            case 'phoneNumber':
                error = value.trim() === '' || phoneNumber.length !== 9 ? 'Please Enter Valid Phone Number' : '';
                break;
    ...
    const { firstName, lastName, email, phoneNumber, currentPassword, newPassword, confirmNewPassword } = formData;
    useEffect(() => {
        getProfile()
    }, [])
                ...
                <Flex mt={4} w={{ base: "100%", lg: "100%" }} h={{ base: "fit-content", xl: "640px" }} bg={'white'} border={"1px solid #cccccc"} rounded={'md'} boxShadow={'md'} flexDir={'column'} gap={2}>
                    <Flex p={{ base: "4", md: "4" }} gap={2} flexDirection={'column'}>
                        <HStack w={'100%'} justify={'space-between'}>
                            ...
                            <Heading w={'80%'} fontSize={'20px'} fontWeight={'500'} className='pc1' primary={"primary"}>Edit Your Profile</Heading>
                            <Box display={{ lg: "none" }}>
                                <ProfileSideBarComponent />
                            </Box>
                        </HStack>
                        <Flex bg={'primary'} flexDir={{ base: "column", md: "row" }} mt={2} gap={4}>
                            <FormControl isInvalid={errors.firstName}>
                                <FormLabel fontWeight={'500'} primary={"primary"}>First Name*</FormLabel>
                                <Input
                                    name="firstName"
                                    value={firstName}
                            ...
                            <FormControl isInvalid={errors.lastName}>
                                <FormLabel fontWeight={'500'} primary={"primary"}>Last Name*</FormLabel>
                                <Input
                                    name="lastName"
                                    value={lastName}
                                    onChange={handleChange}
                        ...
                        <Box py={3} alignSelf={{ base: "center", lg: "end" }}>
                            <Button colorScheme='' variant={'outline'} width={'40'} mr={2} >Cancel</Button>
                            <Button isLoading={loading} textColor={'white'} className='Bg' width='40' onClick={handleSubmit}>Save Changes</Button>
                        </Box>
                    </Flex>
                </Flex>
