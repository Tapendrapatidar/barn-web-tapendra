...
import {
  ...
  TableContainer, Table, Tr, Th, Tbody, Thead, Td,
  Tfoot,
  Button,
  InputGroup,
  Input,
  ...
  return (
    <Box pos={'relative'}>
      ...
      >
        ...
        <Flex mt={4} w={{ base: "100%", lg: "100%" }} overflow={'scroll'} minH={{ base: "80vh", md: "fit-content" }} className='hide-scrollbar' h={{ base: "fit-content", xl: "640px" }} bg={'white'} border={"1px solid #cccccc"} rounded={'md'} boxShadow={'md'} flexDir={'column'} gap={2}>
          <Flex p={{ base: "4", md: "4" }} gap={2} flexDirection={'column'}>
            <HStack w={'100%'} justify={'space-between'}>

              <Heading w={'80%'} fontSize={'20px'} fontWeight={'500'} className='pc1' primary={"primary"}> Order History</Heading>
              <Box display={{ lg: "none" }}>
                <ProfileSideBarComponent />
              </Box>
            </HStack>
                ...
                <>
                  {
                    orderHistory.length <= 0 ?
                      <Box w={'100%'} justifyContent={'center'} display={'flex'} alignItems={'center'}>
...
const MainTable = ({ data, loading }) => {
  ...
  return (
    ...
    <TableContainer minH={'80vh'} p={1}>
      <Table bg={'white'} borderRadius={'10px'}  w={'1200px'} overflow={'hidden'}>
        <Thead >
          <Tr bg={'white'} borderBottom={'2px solid #cccccc'} fontSize={'16px'} >
            ...
            <Th fontWeight={700} border={'none'} textColor={'black'} fontSize={'14px'}>Price</Th>
            <Th fontWeight={700} border={'none'} textColor={'black'} fontSize={'14px'}> Payment Status</Th>
            <Th fontWeight={700} border={'none'} textColor={'black'} fontSize={'14px'}>Status</Th>
          </Tr>
        </Thead>
        ...
        <Tbody>
          {data?.map((item, index) => (
            <Tr cursor={'pointer'} onClick={() => {
              item?.orderStatus === 'unpaid' ? toast.error('Transaction failed') :
                navigate(`/order-tracking?orderId=${item?.orderId}`)
            }} key={index} h={{ base: "14", md: "24" }} m={2} borderRadius={'3px'} bg={'white'} borderBottom={'2px solid #cccccc'} >
              <Td className='hide-scrollbar' overflowY={'scroll'} w={'80'} gap={2}  placeContent={'center'} >
                {
                  item?.orderItems?.map((item, index) => (
                    <HStack key={index} minH={10} mt={1}>
                      <Image w={{ base: "40px", md: "80px" }} h={'10'} fallbackSrc='https://placehold.co/900x400' src={item?.coverImage} alt='img' />
                      <Box>
                        <Text fontSize={{ base: "12px", md: "12px" }} noOfLines={'1'} fontWeight={700}>{item?.name?.slice(0, 28)}{' '} ..</Text>
              ...
              <Td w={'20'} fontWeight={'700'}>{item?.totalItems ? item.totalItems : '-'}</Td>
              {/* <Td w={'20'}>${!item?.originalPrice ? item?.originalPrice * item?.quantity : item?.originalPrice}</Td> */}
              <Td w={'20'}>${item?.shipping?item?.orderTotal+item?.shipping:item?.orderTotal}</Td>
              <Td w={'28'}>
                <Text
                  ...
                  fontWeight={700}
                >
                  {item?.orderStatus.charAt(0).toUpperCase() + item?.orderStatus.slice(1).toLowerCase()}
                </Text></Td>
              <Td>
                <HStack>
                  <Box bg={'#19884A'} h={'12px'} borderRadius={'full'} w={'12px'} />
                  <Text fontSize={{ base: "12px", md: "14px" }} fontWeight={700}>
                    Order Date :  {item?.deliveredAt ? `${formatDate(item.deliveredAt)} ${formatTime(item.deliveredAt)}` : ""}
                  </Text>
                </HStack>
                <Text
                ...
                >
                  {
                    item?.orderStatus === 'paid' ? item?.deliveryStatus.charAt(0).toUpperCase() + item?.deliveryStatus.slice(1).toLowerCase() : ""
                  }
                </Text>
              </Td>
            </Tr>
