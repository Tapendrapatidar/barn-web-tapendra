...
import {
    Box, Text, Flex, FormControl, FormLabel, Input, Checkbox, InputGroup, InputRightElement, Button, IconButton, HStack, PinInput, PinInputField,
} from '@chakra-ui/react';
import { CustomCheckbox, MainSubHeading } from '../../components/shared/custom design/Index';
...
function Login() {
    ...
    if (!isAuth) {
        return (
            <Box px={2} display={'flex'} justifyContent={'center'} alignItems={'center'} pt={10} pb={10} minH={'90vh'} m={'auto'}>
                <Box w={{ base: '100%', md: '500px' }}>
                    ...
                    <Box mt={2} p={2} gap={2} display={'flex'} rounded={'lg'} w={'100%'} h={'14'}>
                        <Button textColor={isActive ? "white" : ""} bg={isActive ? "#2b8f65" : ""} onClick={() => setIsActive(true)} rounded={'full'} w={'50%'} variant={'outline'} colorScheme='' border={'1px solid #cccccc'}> Login with email</Button>
                        <Button textColor={!isActive ? "white" : ""} bg={!isActive ? "#2b8f65" : ""} onClick={() => setIsActive(false)} rounded={'full'} w={'50%'} variant={'outline'} colorScheme='' border={'1px solid #cccccc'}> Login via OTP</Button>
                    </Box>
                    <form>
                        {
                            isActive ?
                                <>
                                    ...
                                    <Box my={4} pt={4}>
                                        <CustomCheckbox isChecked={!rememberMe} defaultChecked={true} setIsChecked={setRememberMe} display='flex'> <Text ml={-1} pb={1}> Remember Me </Text> </CustomCheckbox>
                                    </Box>
                            ...
                            >
                                Remember me
                                        ...
                                        <Link to={'/forgot-password'}>
                                            <Text mt={-10} textColor={'#2b8f65'} >forgot  Password</Text>
                                        </Link>
                                    </HStack>
                                </>
                                ...
                                <>
                                    ...
                                    <FormControl zIndex={9} mt={4} isRequired>
                                        ...
                                        <HStack justify={'space-between'} w={'100% '}>
                                            <HStack j>
                                                <PinInput placeholder='*' otp onChange={(value) => setOtp(value)}>
                                                    <PinInputField />
                                                    <PinInputField />
                                                    <PinInputField />
                                                    <PinInputField />
                                            ...
                                            </HStack>
                                            {
                                                ...
                                                >
                                                    ...
                                                    fontWeight={'semibold'} my={'18px'} textAlign='start' fontSize='14px'
                                                    onClick={handleGetOtp}>
                                                    {resendEnabled ? " " : "Resend OTP"}
                                                    <Button  w={'fit-content'} ml={-4} isDisabled={!resendEnabled} variant={'outline'} colorScheme='' border={'none'}>
                                                        <Text ml={2} cursor={'pointer'} as={'spen'} textColor={'#FF8682'}> {resendEnabled ? "Resend Otp" : <>{
